...
//!
//! Implements the classic Aho-Corasick algorithm with:
//! - Trie construction from a pattern set
//! - Failure link computation via BFS from root
//! - Dictionary/output link computation
//! - Linear-time multi-pattern search
    ...
    fn insert_pattern(&mut self, pattern: &[u8], pattern_id: usize) {
        ...
        for (i, &byte) in pattern.iter().enumerate() {
            ...
            let next = if let Some(&child) = self.nodes[cur].children.get(&b) {
                ...
                self.nodes.push(AcStubNode::new(i + 1));
