...
"#;
    ...
    fn create_engine_from_obj_bytes() {
        let e = WasmEngine::new_from_obj_bytes(SIMPLE_OBJ).unwrap();
        assert_eq!(e.vertex_count(), 3);
    }

    #[test]
    fn build_mesh_bytes_format() {
        let mut e = WasmEngine::new_from_obj_bytes(SIMPLE_OBJ).unwrap();
        let bytes = e.build_mesh_bytes();
        let (version, n_verts, n_idx) = parse_mesh_bytes_header(&bytes).unwrap();
    ...
    fn build_mesh_bytes_correct_length() {
        let mut e = WasmEngine::new_from_obj_bytes(SIMPLE_OBJ).unwrap();
        let bytes = e.build_mesh_bytes();
        let (_, n_verts, n_idx) = parse_mesh_bytes_header(&bytes).unwrap();
    ...
    fn load_target_bytes_works() {
        let mut e = WasmEngine::new_from_obj_bytes(SIMPLE_OBJ).unwrap();
        let target = b"# test\n0 0.1 0.2 0.3\n1 0.0 0.1 0.0\n";
        e.load_target_bytes("height", target).unwrap();
        let bytes = e.build_mesh_bytes();
        assert!(bytes.len() > 12);
    }
    ...
    fn strict_policy_engine_builds() {
        let e = WasmEngine::new_strict(SIMPLE_OBJ).unwrap();
        assert_eq!(e.vertex_count(), 3);
    }
    ...
    fn real_base_mesh_via_wasm_engine() {
        ...
        if let Ok(obj_bytes) = std::fs::read(path) {
            let mut e = WasmEngine::new_from_obj_bytes(&obj_bytes).unwrap();
            assert!(e.vertex_count() > 10_000);
            let bytes = e.build_mesh_bytes();
            let (version, n_verts, _) = parse_mesh_bytes_header(&bytes).unwrap();
            assert_eq!(version, BUFFER_FORMAT_VERSION);
    ...
    fn target_count_increases_on_load() {
        ...
        e.load_target_bytes("weight", target).unwrap();
    ...
    fn import_params_json_round_trips() {
        ...
        let json = e.export_params_json();
        e.reset_params();
        e.import_params_json(&json).unwrap();
...
f 4/1/1 5/2/1 6/3/1
"#;
        let mut e = WasmEngine::new_from_obj_bytes(obj).unwrap();
...
"#;
        ...
        let json = e.get_physics_proxies_json();
        let v: serde_json::Value = serde_json::from_str(&json).unwrap();
        let caps = v["capsules"].as_array().unwrap();
        if !caps.is_empty() {
            for cap in caps {
                assert!(
    ...
    fn set_allowlist_permits_listed_targets() {
        ...
        e.load_target_bytes("muscle", target).unwrap();
        assert_eq!(
            e.target_count(),
            2,
            "muscle should be permitted by allowlist"
        );
        e.load_target_bytes("age", target).unwrap();
    ...
    fn load_zip_pack_bytes_no_targets() {
        ...
        let mut e = WasmEngine::new_from_obj_bytes(SIMPLE_OBJ).unwrap();
        let n = e.load_zip_pack_bytes(&zip).unwrap();
    ...
    fn load_zip_pack_bytes_reinitialises_engine() {
        let mut e = WasmEngine::new_from_obj_bytes(SIMPLE_OBJ).unwrap();
        let t = b"# t\n0 0.05 0.0 0.0\n";
        e.load_target_bytes("old_target", t).unwrap();
        assert_eq!(e.target_count(), 1);
        let zip = make_test_zip("base.obj", SIMPLE_OBJ, &[]);
        let n = e.load_zip_pack_bytes(&zip).unwrap();
        assert_eq!(n, 0);
    ...
    fn load_zip_pack_bytes_invalidates_cache() {
        ...
        let _ = e.build_mesh_bytes();
        assert!(e.has_cached_mesh());
        e.load_zip_pack_bytes(&zip).unwrap();
        assert!(
            !e.has_cached_mesh(),
    ...
    fn list_loaded_targets_contains_names() {
        let mut e = WasmEngine::new_from_obj_bytes(SIMPLE_OBJ).unwrap();
        let t = b"# t\n0 0.1 0.0 0.0\n";
        e.load_target_bytes("height", t).unwrap();
        e.load_target_bytes("weight", t).unwrap();
        let json = e.list_loaded_targets();
        let v: serde_json::Value = serde_json::from_str(&json).unwrap();
    ...
    fn export_quantized_bytes_correct_vertex_count() {
        let mut e = WasmEngine::new_from_obj_bytes(SIMPLE_OBJ).unwrap();
        let bytes = e.export_quantized_bytes();
        let vc = u32::from_le_bytes(bytes[8..12].try_into().unwrap());
        assert_eq!(vc, 3);
    }
    ...
    fn export_quantized_bytes_readable_by_read_quantized_bin() {
        ...
        let mut e = WasmEngine::new_from_obj_bytes(SIMPLE_OBJ).unwrap();
        let bytes = e.export_quantized_bytes();
        let path = std::path::Path::new("/tmp/oxihuman_wasm_qmsh_test.bin");
        std::fs::write(path, &bytes).unwrap();
        let q = read_quantized_bin(path).expect("read_quantized_bin failed");
    ...
    fn get_param_summary_json_reflects_set_params() {
        ...
        let ec = v["extra_count"].as_u64().unwrap();
    ...
    fn get_lod_scene_json_valid_json() {
        ...
        let lod = v["lod_level"].as_u64().unwrap();
        assert_eq!(lod, 1);
    }
    ...
    fn get_loaded_target_names_json_array() {
        ...
        let v: serde_json::Value = serde_json::from_str(&names_json)
            .expect("get_loaded_target_names must return valid JSON");
    ...
    fn get_cloth_state_returns_valid_json() {
        ...
        let v: serde_json::Value =
            serde_json::from_str(&json).expect("cloth state must be valid JSON");
    ...
    fn get_curvature_map_length_matches_vertex_count() {
        ...
        let json = e.get_curvature_map();
        let v: serde_json::Value = serde_json::from_str(&json).unwrap();
        assert_eq!(v.as_array().unwrap().len(), n);
    }
    ...
    fn get_geodesic_distances_source_zero() {
        ...
        let v: serde_json::Value = serde_json::from_str(&json).unwrap();
        let arr = v.as_array().unwrap();
        assert!((arr[0].as_f64().unwrap() as f32).abs() < 1e-5);
    }
    ...
    fn get_mesh_segments_empty_before_build() {
        ...
        let v: serde_json::Value = serde_json::from_str(&json).expect("must be valid JSON");
        assert_eq!(v["segment_count"].as_u64().unwrap(), 0);
    }
    ...
    fn get_mesh_segments_connected_after_build() {
        ...
        let json = e.get_mesh_segments("connected");
        let v: serde_json::Value = serde_json::from_str(&json).expect("must be valid JSON");
        assert!(v["segment_count"].as_u64().unwrap() >= 1);
    }
    ...
    fn step_particles_without_system_returns_empty() {
        ...
        let v: serde_json::Value = serde_json::from_str(&json).expect("must be valid JSON");
        assert_eq!(v["active"].as_u64().unwrap(), 0);
    }
    ...
    fn step_particles_after_create_emits_particles() {
        ...
        let v: serde_json::Value = serde_json::from_str(&json).expect("must be valid JSON");
        let active = v["active"].as_u64().unwrap();
        assert!(
            active >= 1,
